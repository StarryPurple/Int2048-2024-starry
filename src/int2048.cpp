...
int int2048::FFT_NTT_rev[int2048::INT2048_MAX_DIGIT];
...
std::ostream &operator<<(std::ostream &os_object, const int2048 &integer) {
  ...
  std::cout << integer.storage.back();
  for(auto pos_it = ++integer.storage.rbegin(); pos_it != integer.storage.rend(); ++pos_it) {
    int2048::storage_type integer = *pos_it;
    if(integer == 0) os_object << "0000";
    else if(integer < 10) os_object << "000" << integer;
    else if(integer < 100) os_object << "00" << integer;
    else if(integer < 1000) os_object << "0" << integer;
    else std::cout << integer;
  }
  return os_object;
}
...
int2048 &int2048::operator+=(const int2048 &another) {
  ...
  if(carry) storage.push_back(carry);
  return *this;
}
...
void int2048::NTT(std::vector<long long> &vec, int high_bit, int inv, long long mod) {
  ...
  for(int mid = 1; mid < high_bit; mid <<= 1) {
    ...
    for(int j = 0; j < high_bit; j += (mid << 1)) {
      long long w_j = 1;
      for(int k = 0; k < mid; k++, w_j = mult_mod(w_j, ident, mod)) {
        long long l = vec[j + k], r = mult_mod(w_j, vec[j + k + mid], mod);
        vec[j + k] = (l + r) % mod;
  ...
  if(inv == -1) {
    long long inv = pow_mod(high_bit, mod - 2, mod);
    for(int i = 0; i < high_bit; i++)
      vec[i] = mult_mod(vec[i], inv, mod);
...
int2048 &int2048::operator*=(const int2048 &another) {
  ...
  for(int i = 0; i < storage.size(); i++) calc_vec_1[i] = storage[i];
  for(int j = 0; j < another.storage.size(); j++) calc_vec_2[j] = another.storage[j];
  ...
  storage.resize(size_prediction);
  long long carry = 0;
  for(int pos = 0; pos < size_prediction; pos++) {
...
int2048 int2048::inv() {
  ...
  ident.storage[temp_d.storage.size() - 1] = 1;
  temp_c = ident - temp_b;
